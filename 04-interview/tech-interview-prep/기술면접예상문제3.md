...
JPQL은 문자열로 쿼리를 작성하고, QueryDSL은 Java 코드로 작성합니다.
...
Primary Key(기본키)는 테이블에서 각 행을 고유하게 식별합니다.

Foreign Key(외래키)는 다른 테이블의 PK를 참조합니다.
...
정규화는 중복을 제거하고 데이터 무결성을 보장하는 과정입니다.
