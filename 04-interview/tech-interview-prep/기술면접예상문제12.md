...
TDD는 테스트를 먼저 작성하고 그 다음에 코드를 작성하는 개발 방법론입니다.
...
이 시점에는 UserService.register() 메서드가 아예 없으니까 컴파일 에러입니다.
...
1. **버그 조기 발견**  
   테스트를 먼저 쓰니까 요구사항을 명확히 이해하고 시작합니다.

2. **리팩토링 자신감**  
   테스트가 있으니까 코드 변경 후 즉시 검증 가능합니다.

3. **문서화 효과**  
   테스트 코드가 사용법을 보여줍니다.

4. **설계 개선**  
   테스트하기 쉬운 코드는 보통 설계도 좋습니다.
...
완전한 TDD는 어렵지만, 최소한 핵심 비즈니스 로직은 테스트를 먼저 작성하려고 노력합니다.

특히 복잡한 계산이나 조건 분기가 많은 로직은 TDD가 큰 도움이 됩니다.
...
캐싱은 자주 사용하는 데이터를 메모리에 저장해서 빠르게 접근하는 겁니다.
...
SQL Injection은 악의적인 SQL을 삽입해서 DB를 공격하는 기법입니다.
...
JPA, MyBatis는 자동으로 파라미터를 이스케이프합니다.
...
XSS는 악성 스크립트를 삽입해서 다른 사용자의 브라우저에서 실행시키는 공격입니다.
...
다른 사용자가 게시글 보면 쿠키가 공격자 서버로 전송됩니다!
...
JavaScript로 document.cookie 해도 읽을 수 없습니다.
...
`<script>`는 제거되고 `<b>`, `<i>` 같은 것만 허용.

**BizSync 적용:**

React는 기본적으로 안전하지만 사용자 입력을 받는 곳에서 주의:
