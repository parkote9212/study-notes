...
```
INNER JOIN은 양쪽 모두 있는 데이터만 가져오고,
LEFT JOIN은 왼쪽 테이블 전체 + 오른쪽 매칭되는 것만 가져옵니다.
...
주의:
운영 환경에서는 setAllowedOrigins("*") 쓰면 안 됩니다!
구체적인 도메인만 허용해야 합니다.
```
